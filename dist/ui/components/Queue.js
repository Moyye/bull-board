...
const MenuItem = ({ status, count, queue, selectStatus, selected, }) => {
    const selectedStatus = selected ? {} : { [queue.name]: status };
    return (react_1.default.createElement("div", { className: `menu-item ${status} ${selected ? 'selected' : ''} ${count === 0 ? 'off' : 'on'}`, onClick: () => selectStatus(selectedStatus) },
...
exports.Queue = ({ cleanAllDelayed, cleanAllFailed, cleanAllCompleted, queue, retryAll, retryJob, cleanJob, promoteJob, selectedStatus, selectStatus, }) => (react_1.default.createElement("section", null,
    react_1.default.createElement("h3", null, queue.name),
    react_1.default.createElement("div", { className: "menu-list" }, keysOf(constants_1.STATUSES).map(status => (react_1.default.createElement(MenuItem, { key: `${queue.name}-${status}`, status: status, count: queue.counts[status], selectStatus: selectStatus, queue: queue, selected: selectedStatus === status })))),
    selectedStatus && (react_1.default.createElement(react_1.default.Fragment, null,
        ...
        react_1.default.createElement(Jobs_1.Jobs, { retryJob: retryJob, cleanJob: cleanJob, promoteJob: promoteJob, queue: queue, status: selectedStatus })))));
